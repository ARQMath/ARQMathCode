...
def read_latex_tsvs(latex_tsv_directory, dic_formula_id_slt, dic_formula_id_opt, dic_corrected_formula_visual_id,
                    lst_not_found_formula, dic_formula_id_comment_id, result_dir):
    os.mkdir(result_dir)
    ...
    for file in os.listdir(latex_tsv_directory):
        ...
        with open(latex_tsv_directory + "/" + str(file), 'r', newline='', encoding="utf-8") as csv_file:
            ...
            for formula_id in sorted(dic_formula_id_latex):
                ...
                if formula_id in lst_not_found_formula:
                    not_found = True
...
def read_formula_comment_id_file(comment_formula_association):
    ...
    with open(comment_formula_association, 'r', newline='', encoding="utf-8") as csv_file:
        csv_reader = csv.reader(csv_file, delimiter='\t', quoting=csv.QUOTE_ALL)
        for row in csv_reader:
            dic_formula_comment_id[row[0]] = row[1]
...
def main():
    ...
    lst_not_found_formula = read_not_found(not_found_formula)
    lst_not_found_formula.extend(read_not_found(not_in_post))
    lst_not_found_formula.extend(read_not_found(not_in_comment))

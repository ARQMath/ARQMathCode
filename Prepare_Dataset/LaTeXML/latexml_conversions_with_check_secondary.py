...
def get_latex_list(file_path, formula_id_index, latex_index, failed_file):
    dic_formulas = {}
    lst_formula_id = read_failed_formulas(failed_file)
    with open(file_path, newline='', encoding="utf-8") as csv_file:
        csv_reader = csv.reader(csv_file, delimiter='\t', quotechar='"')
        next(csv_reader)
        for row in csv_reader:
            formula_id = row[formula_id_index]
            if formula_id not in lst_formula_id:
                continue
...
def extract_slt_opt_from_latex_file(file_path, formula_id_index, formula_latex_index, failed_file, FORMULA_BATCH_SIZE):
    # read formula_id: latex file
    dic_formula_id_formula_latex = get_latex_list(file_path, formula_id_index, formula_latex_index, failed_file)
...
def convert_tsv_latex_write_output_secondary(latex_file_path, file_id, destination_root, first_round_failures, FORMULA_BATCH_SIZE):
    formula_id_index = 0
    ...
    dic_formula_id_formula_latex, conversion_result_slt, conversion_result_opt, lst_failed, lst_delete, dic_formula_id_with_issues = \
        extract_slt_opt_from_latex_file(latex_file_path + "/" + file_id + ".tsv", formula_id_index, formula_latex_index, first_round_failures + "/failed_" + file_id + ".tsv", FORMULA_BATCH_SIZE)
